...
- Redraw the `<ea-avatar-editor>` canvas when its `canvasSize` input changes; below 200px the dropzone tightens its padding and shrinks the upload icon and text so the prompt still fits.
