...
- Add `<ea-time-picker>`, a stepper-style time picker that pops a popover containing one column per unit (hours, minutes, and an optional seconds column) plus an AM/PM toggle in 12h mode. Each column exposes ChevronUp / ChevronDown buttons and an ARIA-spinbutton value cell that accepts ArrowUp/ArrowDown to step by one (or by the configured `minuteStep` / `secondStep`), PageUp/PageDown for a coarser bump (5× the configured step, 10 for hours), and digit keys to type a value directly. Typing two digits (or a single digit that already saturates the unit) auto-advances focus to the next column; if the second digit would overflow the unit's max, the running value commits first and the new digit starts a fresh buffer. Backspace clears the typed buffer, blur / Tab / arrow keys commit it, and Escape discards it and closes the popover. The wire value emitted by `[(value)]` / `(changed)` is always a 24-hour `"HH:MM"` (or `"HH:MM:SS"` when seconds are included) string regardless of display format, so consumers can swap the trigger label between 12h and 24h without changing the data shape. Supports `[(value)]`, `(changed)`, `label`, `placeholder`, `hint`, `errorMsg`, `disabled`, `readonly`, `required`, `size` (`'sm' | 'md' | 'lg'`), and integrates with Angular forms via `ControlValueAccessor`. Comes with i18n coverage in all five locales.
...
[0.8.1]: https://github.com/mwiraszka/eagami/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/mwiraszka/eagami/compare/v0.7.4...v0.8.0
