...
- `<ea-icon-pencil>` / `PencilIconComponent` is deprecated and will be removed in v2.0.0. The icon is an alias for Feather's `edit-2` (identical path data); use `<ea-icon-edit-2>` / `Edit2IconComponent` instead so the canonical Feather slug is preserved. The deprecated component still exports for backwards compatibility but is no longer listed on the `/ui/icons` reference page
