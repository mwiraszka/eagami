...
export const ru: WebMessages = {
  ...
  home: {
    ...
    contact: {
      ...
      errorMessage:
        'Извините, что-то пошло не так. Напишите напрямую на michal@eagami.com.',
    },
  },
  notFound: {
  ...
  ui: {
    ...
    changelog: {
      ...
      metaTitle: 'Eagami | История изменений',
    ...
    index: {
      ...
      principles: [
        ...
        {
          title: 'Локализация',
          body: 'Встроенный текст компонентов поставляется на двенадцати языках.',
        },
        {
          title: 'Современность',
      ...
      getStartedBefore: 'Перейдите к разделу',
      getStartedLink: 'Установка',
      ...
      getStartedAfter: ', чтобы установить пакет и подключить глобальную таблицу стилей.',
      showcase: {
        button: 'Нажми меня',
    ...
    tokens: {
      ...
      title: 'Дизайн-токены',
      lede: 'CSS-переменные, которые управляют каждым компонентом библиотеки: цвета, типографика, отступы, возвышение, форма и движение. Ссылайтесь на эти токены в своих стилях через <code>var(--token-name)</code>, чтобы сохранить визуальную согласованность во всём приложении.',
      sections: {
        theming: 'Темизация',
      ...
      themingRootBefore:
        'Переопределите любой токен на <code>:root</code>, чтобы перетемизировать всю библиотеку:',
      themingScopedBefore:
    ...
    i18n: {
      metaTitle: 'Eagami | UI | Интернационализация',
      metaDescription:
        'Встроенный текст компонентов на двенадцати языках с переключением во время выполнения и переопределением отдельных строк.',
      title: 'Интернационализация',
      lede: 'Каждая встроенная строка (ARIA-метки, плейсхолдеры, пустые состояния, элементы управления выбором даты) поставляется на двенадцати языках. Задайте один для всего приложения, переключайте во время выполнения или переопределяйте отдельные строки.',
    ...
    component: {
      ...
      playground: {
        ...
        descriptions: {
          ...
          'eagami-wordmark': {
            variant:
              'Вариант содержимого: default это голый логотип-надпись, byline добавляет строку «handcrafted-by», tagline добавляет слоган.',
            layout: 'Размещает логотип-надпись в столбик по строкам или в одну строку.',
            size: 'Значение в пикселях, от которого масштабируется весь логотип-надпись.',
          },
          'empty-state': {
          ...
          tooltip: {
            ...
            eaTooltip:
              'Текстовое содержимое подсказки, отображаемой при наведении и фокусе с клавиатуры.',
          ...
          'date-picker': {
            ...
            placeholder: 'Плейсхолдер, отображаемый на триггере, пока дата не выбрана.',
            readonly: 'Делает поле только для чтения, не давая календарю открываться.',
            required: 'Помечает поле как обязательное.',
            size: 'Визуальный размер триггера выбора даты.',
            ...
            focus: 'Перемещает фокус клавиатуры на кнопку-триггер.',
            onPopoverCloseRequested:
          ...
          drawer: {
            animated: 'Выдвигает панель с её края при открытии выдвижной панели.',
            ariaLabel:
            ...
            width: 'Ширина выдвижной панели вдоль её основной оси.',
            closed:
              'Срабатывает, когда панель закрывается, будь то кнопкой закрытия, затемнением или Escape.',
            opened: 'Срабатывает, как только панель показана через showModal().',
          },
          'data-table': {
            clickable:
          ...
          'tree-node': {
            collapseLabel: 'Доступная метка для кнопки-шеврона свёртывания.',
            disabled: 'Отключает взаимодействие с узлом и его потомками.',
            expandedIds: 'Набор id узлов, которые сейчас раскрыты.',
            expandLabel: 'Доступная метка для кнопки-шеврона раскрытия.',
            focusedId:
              'Id узла, который сейчас удерживает перемещающийся фокус tabindex.',
            level:
              'Глубина от корня дерева (с индексом от 0), используется для отступа и aria-level.',
            node: 'Объект данных, описывающий этот узел, включая его id, метку, потомков и состояние отключения.',
            posInSet:
              'Позиция (с индексом от 1) среди потомков родительского узла, используется для aria-posinset.',
            selectedId: 'Id текущего выбранного узла или null, когда ничего не выбрано.',
            setSize:
              'Общее число соседей в списке потомков родительского узла, используется для aria-setsize.',
            select: 'Срабатывает, когда пользователь кликает или активирует строку узла.',
            toggle:
              'Срабатывает с id узла, когда пользователь кликает шеврон раскрытия или свёртывания.',
          },
          tree: {
