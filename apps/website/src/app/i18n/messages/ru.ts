  ...
  ui: {
    ...
    component: {
      ...
      playground: {
        ...
        descriptions: {
          'bar-chart': {
            labels: 'Подписи категорий вдоль оси категорий, по одной на значение.',
            series:
              'Отображаемые ряды, каждый с именем, одним значением на подпись и необязательным цветом.',
            orientation: 'Направление, в котором столбцы отходят от базовой линии.',
            stacked:
              'Объединяет ряды каждой подписи в один столбец вместо того, чтобы группировать их рядом.',
            showGrid: 'Рисует линии сетки на каждом делении оси значений.',
            showLegend:
              'Показывает легенду под диаграммой, когда на ней больше одного ряда.',
            showValues:
              'Выводит значение каждого столбца у его конца, а в режиме накопления итог каждого стека.',
            height: 'Высота области построения в пикселях; ширина заполняет контейнер.',
            size: 'Визуальный размер текста оси, легенды и подсказки.',
            animation:
              'Анимация появления, повторяемая при каждом изменении данных: рост от базовой линии, каскад по одной подписи за другой, плавное появление или без анимации.',
            animationDuration: 'Длительность анимации появления в миллисекундах.',
            formatValue:
              'Форматирует значения на оси, в подсказке и в таблице данных; по умолчанию группируются согласно активной локали.',
            pointClick:
              'Срабатывает, когда по столбцу щёлкают или выбирают его клавишей Enter или пробелом.',
            activePointChange:
              'Срабатывает, когда выделенный столбец меняется указателем или с клавиатуры, со значением null после снятия выделения.',
          },
          'line-chart': {
            labels: 'Подписи категорий вдоль оси x, по одной на значение.',
            series:
              'Отображаемые ряды, каждый с именем, одним значением на подпись (null оставляет разрыв) и необязательным цветом.',
            curve: 'Как линия изгибается между точками.',
            showArea: 'Заливает область под каждой линией светлым оттенком её цвета.',
            showPoints: 'Отмечает каждое значение точкой.',
            showGrid: 'Рисует горизонтальные линии сетки на каждом делении оси y.',
            showLegend:
              'Показывает легенду под диаграммой, когда на ней больше одного ряда.',
            yMin: 'Нижняя граница оси y; вычисляется по данным, если не задана.',
            yMax: 'Верхняя граница оси y; вычисляется по данным, если не задана.',
            height: 'Высота области построения в пикселях; ширина заполняет контейнер.',
            size: 'Визуальный размер текста оси, легенды и подсказки.',
            animation:
              'Анимация появления, повторяемая при каждом изменении данных: прорисовка каждой линии, раскрытие слева направо, плавное появление, подъём от базовой линии или без анимации.',
            animationDuration: 'Длительность анимации появления в миллисекундах.',
            formatValue:
              'Форматирует значения на оси, в подсказке и в таблице данных; по умолчанию группируются согласно активной локали.',
            pointClick:
              'Срабатывает, когда по точке щёлкают или выбирают её клавишей Enter или пробелом.',
            activePointChange:
              'Срабатывает, когда выделенная точка меняется указателем или с клавиатуры, со значением null после снятия выделения.',
          },
          'pie-chart': {
            data: 'Сегменты, рисуемые по часовой стрелке от отметки двенадцати часов, каждый с подписью, значением и необязательным цветом.',
            variant: 'Сплошной круг или кольцо с пустым центром.',
            showLegend: 'Показывает под диаграммой легенду всех сегментов.',
            showPercentages:
              'Добавляет к записи легенды каждого сегмента его долю от общей суммы.',
            height:
              'Наибольший диаметр диаграммы в пикселях; она уменьшается, чтобы поместиться в более узкий контейнер.',
            size: 'Визуальный размер текста легенды и подсказки.',
            animation:
              'Анимация появления, повторяемая при каждом изменении данных: развёртка по часовой стрелке, рост из центра, плавное появление или без анимации.',
            animationDuration: 'Длительность анимации появления в миллисекундах.',
            formatValue:
              'Форматирует значения в подсказке и в таблице данных; по умолчанию группируются согласно активной локали.',
            sliceClick:
              'Срабатывает, когда по сегменту щёлкают или выбирают его клавишей Enter или пробелом.',
            activeSliceChange:
              'Срабатывает, когда выделенный сегмент меняется указателем или с клавиатуры, со значением null после снятия выделения.',
          },
          timeline: {
          ...
          toast: {
            ...
            show: 'Показывает тост и возвращает его id. Его сообщение и заголовок принимают обычную строку или сегменты с акцентом.',
            success: 'Показывает тост об успехе и возвращает его id.',
            error: 'Показывает тост об ошибке и возвращает его id.',
            warning: 'Показывает тост с предупреждением и возвращает его id.',
            info: 'Показывает информационный тост и возвращает его id.',
            dismiss: 'Удаляет тост с указанным id, если он ещё показан.',
            clear: 'Удаляет все показанные сейчас тосты.',
            pause:
              'Приостанавливает все отсчёты автоматического закрытия, сохраняя оставшееся время каждого тоста.',
            resume: 'Возобновляет отсчёты, приостановленные pause.',
          },
          input: {
            spellcheck:
              'Значение нативного атрибута spellcheck, а без него действует настройка браузера по умолчанию.',
            keepIcon:
              'Сохраняет ведущий значок видимым, пока в поле есть значение; при false значок отображается только вместе с плейсхолдером.',
            role: 'Роль ARIA, применяемая к нативному полю, например combobox для typeahead.',
            ariaExpanded:
              'Состояние раскрытия управляемого всплывающего списка, передается как aria-expanded.',
            ariaControls:
              'id всплывающего элемента, которым управляет поле, передается как aria-controls.',
            ariaActivedescendant:
              'id активного пункта в управляемом всплывающем списке, передается как aria-activedescendant.',
            ariaAutocomplete:
              'Подсказка поведения автозаполнения, передается как aria-autocomplete.',
            labelIcon:
              'Необязательный компонент значка, отображаемый перед текстом подписи.',
          ...
          badge: {
            ...
            uppercase: 'Отображает содержимое полностью заглавными буквами.',
          },
          button: {
            ...
            uppercase: 'Отображает метку полностью заглавными буквами.',
            ariaLabel:
          ...
          checkbox: {
            truncate:
              'Обрезает слишком длинную подпись многоточием и показывает полный текст во всплывающей подсказке.',
          ...
          'color-picker': {
            compact:
              'Показывает триггер как простой образец цвета, без значения и кнопки очистки.',
            disabled: 'Отключает поле.',
            errorMsg:
              'Сообщение об ошибке под полем, заменяет подсказку и помечает поле недействительным.',
          ...
          paginator: {
            ...
            showAllOption:
              'Предлагает после вариантов размер страницы «Все», передавая при выборе PAGE_SIZE_ALL как размер страницы.',
            pageSizeLabel:
              'Что считает выбор размера страницы: локализованную подпись «Строк», «Элементов», «Результатов», «Товаров», «Записей», «Позиций», «Публикаций» или «Статей» на странице.',
          },
          'progress-bar': {
          ...
          slider: {
            ...
            snapValues:
              'Значения, к которым ползунок примагничивается при перетаскивании; каждое отмечено насечкой на шкале.',
          ...
          tag: {
            color:
              'Цвет заливки в шестнадцатеричном виде: красит тег и подбирает читаемый цвет текста.',
            ...
            tooltip:
              'Где подпись, обрезанная maxWidth, показывает полный текст: над тегом или под ним, либо none, чтобы скрыть его.',
            removable:
          ...
          tooltip: {
            tooltipOpen:
              'Программно показывает или скрывает подсказку для триггера, на который не наводят указатель и не ставят фокус; null оставляет это наведению и фокусу.',
            whenClipped:
              'Показывает подсказку, только пока триггер обрезает собственное содержимое.',
            flip: 'Переносит подсказку на противоположную сторону, если с запрошенной нет места.',
            maxWidth:
              'Максимальная ширина в пикселях; текст переносится на этой ширине (нижний предел 50px). Подсказка никогда не выходит за пределы окна просмотра.',
            dismissDelay:
              'Задержка в миллисекундах перед скрытием прокручиваемой подсказки после ухода указателя, достаточная, чтобы навести на неё. Подсказки, которые помещаются, скрываются сразу.',
            eaTooltip:
          ...
          autocomplete: {
            ...
            options:
              'Полный список вариантов, доступных для фильтрации и выбора, плоский или разделённый на группы.',
            placeholder: 'Плейсхолдер, отображаемый, пока поле пустое.',
            readonly: 'Делает поле только для чтения.',
            required: 'Помечает поле как обязательное.',
            size: 'Визуальный размер поля.',
          ...
          'command-palette': {
            disabledWhen:
              'Необязательный предикат, отключающий каждый элемент, для которого он возвращает true, в дополнение к собственному флагу disabled элемента.',
            emptyMessage:
          ...
          'multi-select': {
            created: 'Срабатывает с введённым текстом, когда выбрана строка создания.',
            allowCreate:
              'Предлагает строку создания, когда текст поиска не совпадает ни с одной опцией.',
            ...
            popoverMaxHeight:
              'Максимальная высота списка вариантов в px, после которой список прокручивается.',
            popoverMaxWidth:
              'Максимальная ширина списка вариантов в px или anchor, чтобы ограничить её шириной поля.',
            options:
              'Список выбираемых вариантов, отрисовываемых в выпадающем списке, плоский или разделённый на группы.',
          ...
          'data-table': {
            nowrap:
              'Держит каждую ячейку в одну строку, поэтому в узком окне таблица прокручивается вбок, а не переносит текст.',
            rowHref:
              'Задаёт каждой строке цель ссылки, размещённую настоящей ссылкой в каждой ячейке, чтобы браузер показывал и мог открыть её, а обычный щелчок по-прежнему вызывал rowActivate. Строка, для которой она возвращает null, остаётся инертной: без ссылки, подсветки при наведении, фокуса и активации.',
            sizingRows:
              'Строки с самым широким содержимым каждого столбца, которые размещаются, но никогда не отображаются, чтобы ширина столбцов не менялась между страницами и во время загрузки данных.',
          ...
          'field-label': {
            ...
            icon: 'Необязательный компонент значка, отображаемый перед текстом подписи.',
            labelId:
          ...
          dialog: {
            closeRequested:
              'Срабатывает при manualClose на каждую попытку закрытия, не закрывая диалог.',
            manualClose:
              'Сообщает о каждом закрытии через closeRequested и оставляет диалог открытым, пока open не станет false.',
            modal:
              'Показывает диалог как модальный с подложкой; при false он отображается немодально, и страница позади остаётся прокручиваемой и интерактивной.',
            closeDisabled:
              'Отключает встроенную кнопку закрытия, например пока идёт сохранение.',
            ariaLabel:
