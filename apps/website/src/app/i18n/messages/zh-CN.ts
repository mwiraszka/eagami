    ...
    component: {
      ...
      playground: {
        ...
        descriptions: {
          ...
          'number-input': {
            allowNegative: '是否允许负值；为 false 时值的下限为 0。',
            label: '渲染在字段上方的文本标签。',
            placeholder: '字段为空时显示的占位符。',
            size: '字段的视觉尺寸。',
            hint: '显示在字段下方的辅助文本，在显示错误时隐藏。',
            ...
            readonly: '将字段渲染为只读。',
            required: '将字段标记为必填。',
            min: '最小值；失焦时会将输入的值钳制到该值，步进按钮也会遵循该值。',
            max: '最大值；失焦时会将输入的值钳制到该值，步进按钮也会遵循该值。',
            step: '每一步（方向键或步进按钮）增加或减少的量。',
            id: '应用于原生输入和 label for 的 id，省略时自动生成。',
            value: '当前字段值，为空时为 null，可通过 [(value)] 双向绑定。',
            changed: '每当值变化时携带新值触发。',
            focused: '当字段获得焦点时触发。',
            blurred: '当字段失去焦点时触发。',
            focus: '将键盘焦点移到底层原生字段。',
          },
